...
export default function StatisticsPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([votesData, candidatesData, positionsData, collegesData, votersData]) => {
      setVotes(votesData);
  ...
  const candidateReport = Object.values(
    filteredVotes.reduce((acc, vote) => {
      const candidate = candidates.find(c => c.CandidateID === vote.CandidateID) || {};
      const positionObj = positions.find(p => p.PositionID === candidate.PositionID) || {};
      const collegeObj = colleges.find(c => c.CollegeOfficeID === candidate.CollegeOfficeID) || {};
  ...
  const exportExcel = () => {
    ...
    saveAs(new Blob([buf], { type: "application/octet-stream" }), "candidates-report.xlsx");
  };
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text("Candidates Report", 14, 16);
    autoTable(doc, {
      head: [
        ["First Name", "Surname", "Position", "Party", "College", "Total Votes"]
      ],
  ...
  const exportVoteListExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Vote List Report");
    const buf = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([buf], { type: "application/octet-stream" }), "vote-list-report.xlsx");
  };
  const exportVoteListPDF = () => {
    const doc = new jsPDF();
    doc.text("Vote List Report", 14, 16);
    autoTable(doc, {
      head: [
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4 mb-6">
            <Select value={college} onValueChange={val => setCollege(val || "")}>
              ...
              <SelectContent>
                ...
                {colleges.map((col) => (
                  <SelectItem key={col.CollegeOfficeID} value={String(col.CollegeOfficeID)}>{col.CollegeOffice}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Select value={position} onValueChange={val => setPosition(val || "all")}>
              <SelectTrigger className="w-48">
              ...
              <SelectContent>
                ...
                {positions.map((pos) => (
                  <SelectItem key={pos.PositionID} value={String(pos.PositionID)}>{pos.Position}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <Card className="mb-8 p-6">
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div className="flex flex-row gap-2 items-end">
                ...
                <Button onClick={exportExcel} disabled={candidateReport.length === 0}>Export Excel</Button>
            ...
            <div className="flex flex-row gap-2 items-end">
              ...
              <Button onClick={exportVoteListExcel} disabled={voteListReport.length === 0}>Export Excel</Button>
              <Button onClick={exportVoteListPDF} disabled={voteListReport.length === 0}>Export PDF</Button>
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              <ChartContainer config={{}}>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie data={turnoutData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label />
          ...
          <div className="overflow-x-auto">
            ...
            {!loading && filteredCandidateReport.length === 0 && <div className="text-center py-4">No results found.</div>}
          </div>
          </Card>
        </CardContent>
      </Card>
